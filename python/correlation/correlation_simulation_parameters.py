...
drivers_data_consistency = {
    "Verstappen": {"mean": 0.637, "std_dev": 0.1, "min_lap_time_variation": 0.00, "mean_tyre_change_time": 3.0,
                   "std_dev_tyre_change_time": 0.1},
    "Ricciardo": {"mean": 0.0, "std_dev": 0.1, "min_lap_time_variation": 0.00, "mean_tyre_change_time": 3.0,
                  "std_dev_tyre_change_time": 0.1},
    "Hamilton": {"mean": 0.0, "std_dev": 0.1, "min_lap_time_variation": 0.00, "mean_tyre_change_time": 3.0,
                 "std_dev_tyre_change_time": 0.1},
    "Bottas": {"mean": 0.1, "std_dev": 0.1, "min_lap_time_variation": 0.00, "mean_tyre_change_time": 3.0,
               "std_dev_tyre_change_time": 0.1},
    "Vettel": {"mean": 0.0, "std_dev": 0.1, "min_lap_time_variation": 0.00, "mean_tyre_change_time": 3.0,
               "std_dev_tyre_change_time": 0.1},
    "Raikkonen": {"mean": 0.307, "std_dev": 0.1, "min_lap_time_variation": 0.00, "mean_tyre_change_time": 3.0,
                  "std_dev_tyre_change_time": 0.1},
}
...
correlation_race_config = RaceConfiguration(
...
correlation_drivers: dict = {
    driver: Driver(driver, driver_models_new[driver], drivers_data_consistency[driver], number_of_stops=3,
                   race_configuration=correlation_race_config) for driver in driver_models_new}

# just setting the selected strategy the one i want (TUM) not the optimastion result
for driver in correlation_drivers:
    correlation_drivers[driver].selected_strategy = driver_strategies_correlation[driver]
    if driver == "Bottas":
        correlation_drivers[driver].agent_controlled = True
