...
@dataclass
class DriverVariabilityParams:
    min_lap_time_variation: float
    lap_time_mean_deviation: float
    lap_time_std_dev: float
    mean_tyre_change_time: float
    std_dev_tyre_change_time: float
...
class Driver:
    ...
    def __init__(self, driver_name: str, tyre_models: dict[str, TyreModel],
                 driver_consistency: dict,
                 number_of_stops: int,
        ...
        self._lap_time_mean_deviation = driver_consistency.get('mean', 0.25)
        self._lap_time_std_dev = driver_consistency.get('std_dev', 0.15)
        self._min_lap_time_variation = driver_consistency.get('min_lap_time_variation', 0.05)
        self._mean_tyre_change_time = driver_consistency.get('mean_tyre_change_time', 2.5)
        self._std_dev_tyre_change_time = driver_consistency.get('std_dev_tyre_change_time', 0.3)

        self.driver_variabilty_params = DriverVariabilityParams(self._min_lap_time_variation,
                                                                self._lap_time_mean_deviation,
                                                                self._lap_time_std_dev,
                                                                self._mean_tyre_change_time,
                                                                self._std_dev_tyre_change_time)

        self.strategy_generator = StrategyCombinatorics(self.tyre_models, self._num_of_stops,
        ...
        self.selected_strategy = self.strategy_generator.best_strategy()
        self._lambdified_tyre_models = self.strategy_generator.get_lambda_tyre_models_expr()
        self.available_tyres_method = available_tyres_method

        self.tyre_models_baseline = self.get_base_tyre_models()
    ...
    def get_base_tyre_models(self):
        ...
        for model_name, expr in self.lambdified_tyre_models.items():
            lap_times = expr(laps).tolist()
            base_models[model_name] = lap_times
    ...
    def available_tyres_constraint(self) -> list[tuple[str, int]] | None:
        ...
        return self.strategy_generator.available_tyres_constraint
    ...
    def available_compounds(self) -> list[str] | None:
        ...
        return self.strategy_generator.available_compounds
    ...
    def laps_used(self) -> list[int] | None:
        ...
        """
        return self.strategy_generator.laps_used
    ...
    def strategy_options(self) -> pd.DataFrame:
        ...
        """
        return self.strategy_generator.strategy_df
    ...
    def _check_compounds_availability(self, strategy_compounds):
        ...
        """
        is_valid, _ = self.strategy_generator.strategy_comparison(None,
                                                        strategy_compounds,
    ...
    def get_unique_strategy_permutations(self, use_alternate_strategies: bool = False) \
        ...
        permutations = self.strategy_generator.strategy_permutations(strategies_compounds)
    ...
    def _update_strategy_dependent_attributes(self, keep_current_strategy: bool = False) -> None:
        ...
        if not keep_current_strategy:  # basically if there is an update to tyre models or culling of tyre,
            ...
            self._selected_strategy = self.strategy_generator.best_strategy()
        self._lambdified_tyre_models = self.strategy_generator.get_lambda_tyre_models_expr()
    ...
    def update_tyre_usage(self, used_tyres: list[tuple[str, int]], current_tyre: str) -> None:
        ...
        """
        self.strategy_generator.update_used_tyres_and_current_tyre(used_tyres, current_tyre)
    ...
    def update_tyre_models(self, new_tyre_models: dict[str, TyreModel]) -> None:
        ...
        """
        self.strategy_generator.update_tyre_models(new_tyre_models)
    ...
    def cull_tyre(self, compound: str) -> None:
        ...
        self.strategy_generator.cull_tyre(compound)
    ...
    def _validate_race_strategy(self, proposed_strategy: tuple):
        ...
        if not all(
                isinstance(compound, str) and isinstance(laps, (int, float)) and self.strategy_generator.check_compound_validity(
    ...
    def __breadth_method(self, top_n: int, stops_limit: int, use_dict_mode: bool, strategies_dict: dict,
        ...
        for stop in range(1, stops_limit + 1):
            ...
            filtered_df = self.strategy_generator.filter_by_stop(stop)
            # get top n (fastest) of those rows
            top_n_fastest_strat_df = self.strategy_generator.find_n_lowest_strategies(
                top_n, filtered_df)
            # get race time and optimal dist of these strategies
            formatted_df = top_n_fastest_strat_df.apply(
                self.strategy_generator.get_racetime_and_optimal_dist, axis=1)
    ...
    def __depth_method(self, top_n: int, stops_limit: int, use_dict_mode: bool, strategies_dict: dict,
        ...
        for stop in range(1, stops_limit + 1):
            ...
            filtered_df = self.strategy_generator.filter_by_stop(
...
class Agent(Driver):
    def __init__(self, driver_name, tyre_models, driver_consistency, number_of_stops, race_configuration, top_n_strategies_parameter = 1, stops_depth_limit = None, alternate_strategies_method = 'breadth', start_compound = None, available_tyres_constraint = None, available_tyres_method = 'least_used'):
        super().__init__(driver_name, tyre_models, driver_consistency, number_of_stops, race_configuration, top_n_strategies_parameter, stops_depth_limit, alternate_strategies_method, start_compound, available_tyres_constraint, available_tyres_method)
