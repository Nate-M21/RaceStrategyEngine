...
from .utility import driver_styles_for_plotting
...
class RaceSimulation(BaseRaceSimulation):
    ...
    def update_driver_race_strategy(self, driver, lap_distribution, optional_message=None):
    ...
    def run_simulation(self) -> None:
        ...
        self._reset_traffic_counter()
        self._reset_accumulated_race_times()
        self._lap_discrete_simulation()

    def _exp_run_simulation(self, show_result = False):
        self._check_exp_backend_on()

        sim_data = self._get_rust_simulation_data()
        result = self._simulation_engine.run_simulation(sim_data, show_results= show_result)
    ...
    def plot_tyre_usage(self):
        ...
        fig.update_layout(
            ...
            yaxis_title='Driver',
            showlegend=False,
            yaxis={'categoryorder': 'array', 'categoryarray': sorted_drivers[::-1]}
            # Reverse order to have winner at the top
        )
...
if __name__ == '__main__':
    ...
    from correlation.correlation_simulation_parameters import correlation_drivers, correlation_race_state, correlation_race_config
    # test_race = RaceSimulation(correlation_drivers, correlation_race_state, correlation_race_config)
    test_race._enable_exp_rust_backend("time_discrete")
    start_time = time.perf_counter()

    # test_race.run_simulation()
    # test_race.display()

    # a = test_race._exp_run_simulation(False)
    # print(a)
    test_race.run_simulation()
    test_race.display()
