...
class StrategyCombinatorics(StrategyOptimization):
    ...
    def get_racetime_and_optimal_dist(row: pd.Series):
        ...
        # Doing this because new numpy update makes the view ugly
        optimal_dist = []
        for compound, laps in row['optimal_dist']:
            optimal_dist.append((compound, float(laps)))

        # Creating the final tuple with the race time first, followed by the tyre distribution
        result_tuple = (float(race_time),) + tuple(optimal_dist)
    ...
    def _validate_available_tyres(self, available_tyres: list[tuple[str, int]] | None) \
        ...
        if len(set(compound for compound, _ in available_tyres)) == 1:
            raise ValueError("Regulations require the use of at least two different compounds during the race. "
                             "You have only entered one for the available tyres")
