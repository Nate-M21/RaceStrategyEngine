...
The child RaceSimulation class should be used for actual simulation purposes. However, for ease of
...
Example:
    ...
    test_race._lap_discrete_simulation()

    # Display the result
    test_race.display()

    # Note: Unlike RaceSimulation, BaseRaceSimulation doesn't provide functionality
...
from real_time_example.data_generator import list_of_race_states, lap_to_index
from .utility import RaceState

# random.seed(20)

DriverSimulationData = namedtuple('DriverSimulationData', ['name', 'starting_position','position', 'strategy',
                                          "driver_current_lap", "driver_race_time","driver_race_progress",
                                          "driver_in_pit_lane", "driver_time_spent_in_pit_lane",
...
class BaseRaceSimulation(ABC):
    ...
    def __init__(self, drivers: dict[str, Driver], starting_race_grid: RaceState, race_configuration: RaceConfiguration):
        """
        ...
        self._race_history_list = {driver: [] for driver in self._race_state}

        self._exp_rust_backend = False

    def _check_exp_backend_on(self):
         if not self._exp_rust_backend:

            raise ValueError("\n\nStop the car!\nStop the car! \nWe have a problem. \nSorry mate, need to retire, stop the car.\n" \
            "This expermintal backend for the simulation and has not been turned on.")


    def _enable_exp_rust_backend(self, simulation_type: Literal["time_discrete", "lap_discrete"] = "lap_discrete", 
                                 
                                track_name = 'abu_dhabi',
                                  time_step: float = 0.1):
        try:
            from strategy_engine_core import SimulationEngine
            from strategy_engine_core import smart_round as rusty_smart_round

            self._simulation_engine = SimulationEngine(self.drivers,self.race_config, simulation_type,track_name, time_step)
            self._rusty_smart_round = rusty_smart_round
            self._exp_rust_backend = True
        except ImportError:
            message = "\nCopy, We are checking\n" + "...\n"*3 + "(crickets)\n" + "...\n"*3
            message += "The 'strategy_engine_core' module was not found.\nEnsure you have Rust and maturin installed. Then " \
            "run 'maturin develop --release' in the root directory to create it."
            message +='\n\nIf on MacOs using Tch / LibTorch and you get a "Library not loaded: @rpath/libtorch_cpu.dylib" try the following below:\n'
            message += 'export TORCH_LIB=$(python -c "import torch, os; print(os.path.join(os.path.dirname(torch.__file__), \'lib\'))")'
            message += '\nThen:\nexport DYLD_LIBRARY_PATH=$TORCH_LIB:$DYLD_LIBRARY_PATH'
            message += '\nIf the error still persists use rust native Candle instead'
            raise ImportError(message)

    def _disable_exp_rust_backend(self):
        self._simulation_engine = None
        self._rusty_smart_round = None
        self._exp_rust_backend = False
    ...
    def _reset_race_history(self) -> None:
        ...
        Notes:
            ...
            same instance to run multiple run consecutive simulations
        """
        # TODO Need to find smarter way to do this! I cant keep making new dataframes.
        self.race_history = pd.DataFrame({driver: [] for driver in self._race_state})

    def _check_overtake(self, driver: str, current_lap: int, precomputed_overtake_penalties) -> None:
                ...
                if driver_ahead is not None:
                    ...
                    if self._sim_race_state[driver]["total_time"] < self._sim_race_state[driver_ahead]["total_time"]:
                        ...
                        # if time_gap > 1:
                            ...
                            failed_overtake_penalty = precomputed_overtake_penalties[driver][current_lap-1]
                            self._sim_race_state[driver]['total_time'] = self._sim_race_state[driver_ahead]['total_time'] + failed_overtake_penalty
    ...
    def _update_positions_post_pit_stops(self, pitted_drivers: list) -> None:
        ...
        # sorted_pitted_drivers = pitted_drivers
    ...
    def _compute_lap_time(self, current_lap: int, driver: str, tyre_model_contribution_to_lap_time: float) -> float:
        ...
        # standard deviation.
        driver_effect = max(driver_object._min_lap_time_variation,
                            random.gauss(driver_object._lap_time_mean_deviation,
                                         driver_object._lap_time_std_dev))
    ...
    def _get_drivers_pit_and_compound_info(self) -> dict:
        ...
        for driver in self._race_state:
            ...
            for compound, laps in zip(compounds, rounded_laps_on_compounds):
                strategy_detail.extend([(i, compound) for i in range(1, laps + 1)])
    ...
    def _get_precomputed_lap_times(self, drivers_pit_and_compound_info):

        min_pen = self.race_config.min_time_lost_due_to_failed_overtake_attempt
        max_pen = self.race_config.max_time_lost_due_to_failed_overtake_attempt
        num_laps = self.race_config.num_laps
        num_drivers = len(self._race_state)
        failed_ovt_penalties = np.random.uniform(min_pen, max_pen, (num_drivers, num_laps)).tolist()

        precomputed_lap_times = {}
        drivers_penalty = {}
        drivers_pit_losses: dict[str, list] = {}

        for index, (driver_name, values) in enumerate(drivers_pit_and_compound_info.items()):
            # print(index, driver_name)
            driver_object = self.drivers[driver_name]
            strategy = values['strategy']
            pit_laps = values['pit_laps']

            total_pit_loss = []

            for pit_lap in pit_laps:
                tyre_change_time = random.gauss(driver_object._mean_tyre_change_time, driver_object._std_dev_tyre_change_time)
                pit_lane_loss = self.race_config.pit_lane_time_loss
                whole_pit_loss = tyre_change_time + pit_lane_loss

                total_pit_loss.append(whole_pit_loss)

            driver_lap_times = []
            for current_lap, (lap_num_on_compound, compound) in enumerate(strategy, start=1):

                initial_lap_time_calc = self._lambdified_drivers_tyre_models[driver_name][f'{compound}'](lap_num_on_compound)


                lap_time = self._compute_lap_time(current_lap, driver_name, initial_lap_time_calc)

                driver_lap_times.append(lap_time)

            precomputed_lap_times[driver_name] = driver_lap_times
            drivers_penalty[driver_name] = failed_ovt_penalties[index]
            drivers_pit_losses[driver_name] = total_pit_loss

        return precomputed_lap_times, drivers_pit_losses, drivers_penalty


    def _extract_strategy_compounds_and_laps(self, driver: Optional[str] = None, optional_strategy = None, included_race_time: bool = True):

        if optional_strategy is None and driver is None:
            raise ValueError("Either 'driver' or 'optional_strategy' must be provided.")
    ...
    def _lap_discrete_simulation(self) -> None:
        ...
        precomputed_lap_times, prec_pit_loss, prec_overtake_penalties = self._get_precomputed_lap_times(self.drivers_pit_and_compound_info)
        ...
        for current_lap in range(self._start_lap, self.race_config.num_laps + 1):
            ...
            for driver in self._sim_race_state:
                ...
                driver_lap_time = self._get_driver_lap_time_and_update_pit_status(driver,
                                                                                  current_lap,
                                                                                  self.drivers_pit_and_compound_info,
                                                                                  precomputed_lap_times,
                                                                                  prec_pit_loss,
                                                                                  pitted_drivers)
            ...
            for driver in sorted_drivers_position:
                ...
                self._check_overtake(driver, current_lap, prec_overtake_penalties)
    ...
    def get_driver_ahead_name(self, position: int):
        driver_ahead = None

        for name, values in self._sim_race_state.items():
    ...
    def _get_driver_lap_time_and_update_pit_status(self, driver: str, current_lap: int, drivers_lap_info: dict,
                                                   precomputed_lap_times: dict, precomputed_pit_loss: dict,
        ...
        lap_time = precomputed_lap_times[driver][current_lap - 1]
        ...
        if current_lap in pit_out_laps:
            ...
            total_pit_loss = precomputed_pit_loss[driver].pop()
    ...
    def _get_laps_on_compound(self, current_lap, driver, drivers_lap_info, stint) -> tuple[int, int]:
        ...
        # The subtract by 1 is so that if a tyre is new (not being used) the lap on compound is 0 not 1
    ...
    def display(self) -> None:
        ...
        for driver in sorted_drivers:
            ...
            print(f'P{ranking}\t{driver[0:3]} \t\t{self._sim_race_state[driver]["total_time"]}')
        print('*#' * 50)
    ...
    def _check_tyre_usage(self):
        ...
        for driver, result in self._sim_race_state.items():
            ...
            if total_laps != self.race_config.num_laps:
                ...
                index_to_use = lap_to_index[self._start_lap]
                file_race_state = list_of_race_states[index_to_use][driver]
                ...
                used_tyres_errors.append(
                    ...
                    f"The race state the file says:\n{file_race_state}\n")
    ...
    def _get_rust_simulation_data(self):
        
        sim_data = []

        for driver, info in self._race_state.items():

            compounds, laps_on_compounds = self._extract_strategy_compounds_and_laps(driver, included_race_time=True)

            # Rounding to ensure the pit laps are integers and not floating points from the optimization method

            rounded_laps_on_compounds = self._rusty_smart_round(laps_on_compounds, self.race_config.num_laps, False)

            strategy = list(zip(compounds, rounded_laps_on_compounds))

            starting_position = self._starting_grid[driver]["position"]
            position = info["position"]



            race_time = info["total_time"]

            lap_times = self._race_history_list[driver]

            driver_data = DriverSimulationData(
                name=driver,
                starting_position=starting_position,
                position=position,
                strategy=strategy,
                driver_current_lap=self._start_lap,
                driver_race_time=race_time,
                driver_race_progress=1.0, # For time discrete, need current lap progress + current lap # TODO
                driver_in_pit_lane =False,# For time discrete, NB field pit_stop_info, need to know when
                #  driver entered the pitlane # TODO
                driver_time_spent_in_pit_lane = 0.0,# For time discrete # TODO
                driver_accumulated_lap_times=lap_times
            )
            sim_data.append(driver_data)
