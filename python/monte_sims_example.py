...
test_race = MonteCarloRaceSimulations(drivers=drivers, starting_race_grid=race_state, race_configuration=race_config, lap_variation_range=(-10,10),
                                      simulation_type="lap_discrete", track_name="abu_dhabi", time_step=1)
...
for i in reversed(range(6)):
    sys.stdout.write(f"\rStarting simulations in: {i}")
    time.sleep(1)
    sys.stdout.flush()
...
df = test_race.run_and_analyze_simulations_from_monte_carlo_runs(
