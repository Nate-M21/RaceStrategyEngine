...
use burn::{
    ...
    tensor::{
        Int, Tensor, activation::softmax, backend::AutodiffBackend, loss::cross_entropy_with_logits,
    },
};
...
unsafe impl<B: Backend + AutodiffBackend> Send for RonModel<B> {}
unsafe impl<B: Backend + AutodiffBackend> Sync for RonModel<B> {}

#[derive(Module, Debug)]
pub struct ModelBackend<B: Backend> {
    pub shared_trunk: Vec<Linear<B>>,
...
impl<B: Backend> ModelBackend<B> {
    ...
    fn forward<const D: usize>(
        ...
        let value_logits = self.value_head.forward(x.clone());

        let position_logits = self.position_head.forward(x);

        (policy_logits, value_logits, position_logits)
    }
...
// R.O.N - Race Optimisation Network
#[derive(Clone)]
pub struct RonModel<B: Backend + AutodiffBackend> {
    model: ModelBackend<B>,
    optimizer_config: SgdConfig,
    optimizer: OptimizerAdaptor<Sgd<B::InnerBackend>, ModelBackend<B>, B>,
}

impl<B: Backend + AutodiffBackend> RonModel<B> {
    ...
    fn _predict(
        ...
        let (policy_logits, value_logits, position_logits) =
            self.model.valid().forward(observation.clone());
        ...
        (action_probabilities, value_logits, position_probabilities)
    }
    ...
    fn calculate_action_and_value(
        ...
        let (action_probabilities, value_logit, _) = self._predict(observation);
...
impl<B: Backend + AutodiffBackend> ActorCritic for RonModel<B> {
        ...
        Environment: crate::traits::gym::MCTSGymEnvironment<Observation = Self::ObservationType>,
    {
        ...
        let observation: Tensor<<B as AutodiffBackend>::InnerBackend, 1> =
            Tensor::from_data(observation, &device);
        let (policy_logits, value_logits, _position_logits) =
            self.model.valid().forward(observation);
        ...
        Environment: crate::traits::gym::MCTSGymEnvironment<Observation = Self::ObservationType>,
    {
        ...
        let (action_probabilities, value_logits) = self.calculate_action_and_value(&observation);
        let device = action_probabilities.device();
    ...
    fn train_model(
        ...
        for iteration in 0..config.training_steps {
            ...
            let value_targets_batch: Tensor<B, 2> = scalar_to_support_batch(&value_targets, device);
            let position_targets: Tensor<B, 1, Int> =
                Tensor::from_data(position_targets.as_slice(), device);

            let (policy_predictions, value_predictions, position_predictions) =
...
impl RonModelConfig {
    pub fn init<B: Backend>(&self, device: &B::Device) -> ModelBackend<B> {
        let mut shared_trunk = Vec::with_capacity(6);
