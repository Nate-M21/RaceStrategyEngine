...
pub fn selfplay<B: Backend + AutodiffBackend>(
    ...
    // test_model(config, environment.clone(), &network);
    // println!();
...
pub fn test_model<B: AutodiffBackend + Backend>(
    ...
    while !game.terminal() && game.history.len() < config.max_moves {
        ...
        (observation, info) = game.apply(ActionOrOutcome::Action(Action::new(action)));
...
fn update_weights_batch<B: Backend + AutodiffBackend>(
    ...
    println!("TOTAL - LOSS = {total_loss_print}");
...
pub struct StochasticMuzero<B: Backend + AutodiffBackend> {
    config: StochasticMuzeroConfig,
    model: Arc<RwLock<Network<B>>>,
    environment: RaceStrategyEnvironment,
    replay_buffer: ReplayBuffer,
}

impl<B: Backend + AutodiffBackend> StochasticMuzero<B> {
    pub fn new(
        config: StochasticMuzeroConfig,
        model: Network<B>,
        environment: RaceStrategyEnvironment,
    ...
    fn play_episode(
        ...
        shared_network: Arc<RwLock<Network<B>>>,
    ) -> Game {
